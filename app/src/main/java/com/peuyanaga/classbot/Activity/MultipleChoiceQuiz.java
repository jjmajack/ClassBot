...
public class MultipleChoiceQuiz extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(Server server : global.getServerList()){
            if (server.getName().equalsIgnoreCase("topic_multiple_choice")){
                ipaddress = server.getIpAddress();
                port = server.getPort();
                break;
            }
        }
        setTitle(subject.getSubjectName() + " - " + topic.getTopic());
        data.setTopicId(topic.getTopicId());

        lblTopic = (TextView)findViewById(R.id.lblTopic);
        questionView = (TextView)findViewById(R.id.question_view);

        lblTopic.setText(subject.getSubjectName() + " - " + topic.getTopic() + " : " + score + "xp");
                    ...
                    while((line = bufferedReader.readLine()) != null)
                    {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                score = serverResponse(data, person, lblTopic, questionView, subject, score, topic, chatMessageList);
